...
Personal blog/portfolio site built with Astro, TypeScript, and SCSS. Brutalist design with dark/light theme support and i18n (EN/ES). Uses React only for the interactive Personal Dashboard (KaTeX + custom SVG charts). Deployed to GitHub Pages at addleonel.github.io via GitHub Actions.
...
- **Pages:** `src/pages/` — EN at root (`/`), ES at `/es/`. Includes `index.astro`, `about.astro`, `posts.astro`, `contact.astro`, `personal.astro`, `neuroemociones.astro`, `posts/[slug].astro`
- **Components:** `src/components/` — `Header.astro`, `Footer.astro`, `PostPreview.astro`, `VectorSpace.astro` (Three.js), `ThemeToggle.astro`, `LanguageSwitcher.astro`, `PersonalDashboard.tsx` (React, loaded with `client:load`)
